...
class AddChildBenefitToIncomeTypeSchema extends Schema {
  async up() {
    await Database.raw('ALTER TABLE incomes DROP CONSTRAINT IF EXISTS incomes_income_type_check;')
    await Database.raw(
      `ALTER TABLE incomes ADD CONSTRAINT incomes_income_type_check check (income_type in ('${INCOME_TYPE_EMPLOYEE}', '${INCOME_TYPE_WORKER}','${INCOME_TYPE_UNEMPLOYED}','${INCOME_TYPE_CIVIL_SERVANT}','${INCOME_TYPE_FREELANCER}','${INCOME_TYPE_HOUSE_WORK}','${INCOME_TYPE_PENSIONER}','${INCOME_TYPE_SELF_EMPLOYED}','${INCOME_TYPE_TRAINEE}', '${INCOME_TYPE_OTHER_BENEFIT}', '${INCOME_TYPE_CHILD_BENEFIT}'));;`
    )
  }

  down() {
    this.table('add_child_benefit_to_income_types', (table) => {
      // reverse alternations
    })
  }
}

module.exports = AddChildBenefitToIncomeTypeSchema
