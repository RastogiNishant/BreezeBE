...
class AdjustMemberMainProfileProspectSchema extends Schema {
  async up() {
    const users = await Database.raw(
      `select u.id, u.firstname, u.secondname, u.email, owner_id, u.status, u.created_at from users u left join members m on u.id = m.user_id where u.role = ${ROLE_USER} and u.owner_id is null and m.id is nulland u.status != ${STATUS_DELETE}`
    )
    let i = 0
