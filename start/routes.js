...
Route.get('/test', async ({ response }) => {
  ...
  const OpenImmoReader = require('../app/Classes/OpenImmoReader')
  ...
  const { THIRD_PARTY_OFFER_PROVIDER_INFORMATION, STATUS_ACTIVE } = require('../app/constants')
  ...
  await Promise.map(properties, async (estate) => {
    ...
    QueueService.getThirdPartyCoords(result.id)
  })
