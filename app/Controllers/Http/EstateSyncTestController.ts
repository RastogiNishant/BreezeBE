...
export class EstateSyncTestController {
   ...
   */
  static async generateEstateData({ request, response }): Promise<void> {
    const { estateId } = request.all()

    let estate: EstateWithDetails = (await EstateService.getByIdWithDetail(estateId)).toJSON()
   ...
   * Testing data update
   */
  static async updateProperty({ request, response }): Promise<void> {
    const { estateId } = request.all()
   ...
   */
  static async publishProperty({ request, response }): Promise<void> {
    const { estateId } = request.all()
   ...
   */
  static async removeProperty({ request, response }): Promise<void> {
    ...
    if (propertyId !== undefined) {
      ...
      let listingArr = listings ? listings.data : []
      // @ts-ignore
      // response.res(Object.keys(listings.data))
      // return
