...
class FilterColumnsService {
  ...
  static async update(id, data) {
    return await FilterColumns.query()
      .where('id', id)
      .update({ ...data })
  }
  ...
  static async getAll({ user_id = null, filter }) {
    let query = FilterColumns.query().select('filter_columns.*')

    if (user_id) {
      query
        .select(
          Database.raw('COALESCE(_fcp.order, filter_columns.order, 1) as real_order'),
          Database.raw('COALESCE(_fcp.visible, filter_columns.default_visible, false) as visible')
        )
    ...
    if (filter.is_used_filter) {
      query.whereIn(
        'is_used_filter',
        Array.isArray(filter.is_used_filter) ? filter.is_used_filter : [filter.is_used_filter]
      )
    }

    if (user_id) {
      query.orderBy('real_order', 'asc')
    }
