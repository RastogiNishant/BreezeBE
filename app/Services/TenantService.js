...
class TenantService {
  ...
  static async activateTenant(tenant) {
    ...
    const schema = yup.object().shape({
      ...
      unpaid_rental: yup.number().positive().oneOf([NO_UNPAID_RENTAL, YES_UNPAID_RENTAL, NO_ANSWER_UNPAID_RENTAL]).required(),
      insolvency_proceed: yup.number().positive().oneOf([NO_INSOLVENCY, YES_INSOLVENCY, NO_ANSWER_INSOLVENCY]).required(),
      arrest_warranty: yup.number().oneOf([NO_ARREST_WARRANTY, YES_ARREST_WARRANTY,NO_ANSWER_WARRANTY]).required(),
      clean_procedure: yup.number().oneOf([NO_CLEAN_PROCEDURE, YES_CLEAN_PROCEDURE,NO_ANSWER_CLEAN_PROCEDURE]).required(),
      income_seizure: yup.number().oneOf([NO_INCOME_SEIZURE,YES_INCOME_SEIZURE,NO_ANSWER_INCOME_SEIZURE]).required(),
