...
class MatchService {
  ...
  static filterEstates(tenant, estates) {
    ...
    //Logger.info(`tenant info ${JSON.stringify(tenant)}`)
    const minTenantBudget = ((tenant.income || 0) * (tenant?.budget_min || 0)) / 100
    const maxTenantBudget = ((tenant.income || 0) * (tenant?.budget_max || 100)) / 100
  ...
  static async createNewMatches({ tenant, only_count = false, has_notification_sent = true }, trx) {
    ...
    estates = this.filterEstates(tenant, estates)
