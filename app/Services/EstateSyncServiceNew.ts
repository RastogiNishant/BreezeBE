...
import * as EstateService from './EstateService'
    ...
    if (resp?.success) {
      ...
      try {
        ...
        await require('./MailService').sendEmailToOhneMakler(
          `Error on the Post response: ${resp?.success ? 'success' : 'failed'} ` + JSON.stringify(resp),
          'barudo@gmail.com'
        )
      } catch (exc) {}

      throw new HttpException(`Error on the Post response: ${resp?.success ? 'success' : 'failed'} ` + JSON.stringify(resp))
    }
  },
      ...
      await BluebirdPromise.map(
        listings.rows.map(
          async (listing: { provider: string, updateItem: (data: any) => Promise<void> }) => {
            ...
            trace += `\nPOST TO LISTING: ${String(target.estate_sync_target_id)}, ${propertyId}`
            ...
            if (listingResponse.success) {
              ...
              if (listingResponse?.data?.message !== undefined) {
                trace += `\nError found while posting: ${String(listingResponse?.data?.message)}`
                await listing.updateItem({
                  status: ESTATE_SYNC_LISTING_STATUS.ERROR_FOUND,
                  publishing_error: true,
